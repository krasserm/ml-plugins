...
def _get_type_str(col_info: object) -> str:
    """Convert a datasets-server feature descriptor to a readable type string.

    Handles the HF feature shapes: a `Value`/`ClassLabel`/`Sequence` dict, a
    bare list (a list/sequence of a sub-feature, e.g. an SFT `messages` column
    arrives as ``[{...}]``), and a nested struct (a dict of named sub-features).
    Never raises on an unexpected shape — returns "unknown" instead — so a
    single list-typed column cannot drop the whole schema table.
    """
    match col_info:
        case list():
            inner = col_info[0] if col_info else {}
            return f"list<{_get_type_str(inner)}>"
        case dict() as ci:
            if ci.get("_type") == "ClassLabel":
                names = ci.get("names", [])
                if names and len(names) <= 5:
                    return f"ClassLabel ({', '.join(f'{n}={i}' for i, n in enumerate(names))})"
                return f"ClassLabel ({len(names)} classes)"
            if ci.get("_type") == "Sequence":
                return f"list<{_get_type_str(ci.get('feature', {}))}>"
            dtype = ci.get("dtype") or ci.get("_type")
            if dtype:
                return str(dtype)
            if ci:  # a struct: dict of named sub-features
                return "struct{" + ", ".join(ci.keys()) + "}"
            return "unknown"
        case _:
            return "unknown"
